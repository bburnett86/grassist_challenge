GRASSIST.lawnmower = function(homeLongitude, homeLatitude) {
    ...
    var addToSeen = function(string) {
        if (!seenArray.includes(string)) {
            seenArray.push(string)
        }
    }

    var seenAlready = function(string) {
        if (seenArray.includes(string)) {
            return true
        } else {
            return false
    ...
    var removeFromSeen = function(string) {
        seenArray.splice(seenArray.indexOf(string), 1);
    }
    ...
    that.removeFromSeen = removeFromSeen;
    that.showSeenArray = showSeenArray;
    that.seenAlready = seenAlready;
    that.addToSeen = addToSeen;
    that.seenArray = []
